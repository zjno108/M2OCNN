...
class VitResBlock(nn.Module):
    def __init__(
        ...
        self.detail_dec = nn.Sequential(
            # 1x1
            nn.Conv2d(dim, h_dim, kernel_size=1),
            nn.BatchNorm2d(h_dim),
            nn.GELU(),
            # 3x3
            # nn.ReflectionPad2d(1),
            nn.ConvTranspose2d(h_dim, h_dim, 3, padding=1),
            nn.BatchNorm2d(h_dim),
            nn.GELU(),
            # 3x3
            # nn.ReflectionPad2d(2),
            nn.ConvTranspose2d(h_dim, h_dim, 5, padding=2),
            nn.BatchNorm2d(h_dim),
            nn.GELU(),
            # 1x1
            nn.Conv2d(h_dim, dim, kernel_size=1),
            nn.BatchNorm2d(dim),
        )

    def forward(self, x):
        x = x + self.struct_dec(x)
        x = x + self.detail_dec(x)
